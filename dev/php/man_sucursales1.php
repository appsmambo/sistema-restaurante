<?php
	...
	if($id > 0){
		...
		$consulta = mysqli_query($con, "select * from man_sucursales where id_sucursal='$id'");
		if(mysqli_num_rows($consulta) > 0){
			...
			$administrador = $row[id_administrador];
		}
	}
?>
<!DOCTYPE html>
...
<script language="javascript">
	function eliminar(a){
		location.href = "../sql/sucursales_eliminar.php?id="+a;
	}

	function buscar_proveedores(a,b){
		$.ajax({
			url: "../sql/sucursales_existe.php",
			type: "POST",
			data: "valor="+a+"&id="+b,
      ...
      <div class="row">
        ...
        <div class="col-md-6">
          ...
          <div class="box box-primary">
            ...
            <!-- /.box-header -->
            <!-- form start -->
            <form role="form" id="form1" name="form1" action="../sql/sucursales_grabar.php" method="post">
              <div class="box-body">
                <div class="form-group">
                  <label for="exampleInputEmail1">Nombre (*)</label>
                  <input name="txtNombre" type="text" class="form-control" id="txtNombre" style="text-transform:uppercase" value="<?php echo $nombre;?>" maxlength="200" placeholder="" autocomplete="off" >
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                  ...
                  <select class="form-control select2" id="txtAdministrador" name="txtAdministrador" style="width: 100%" >
                    <option value=""></option>
					<?php
						@mysqli_query($con, "SET NAMES 'utf8'");
						$consulta = mysqli_query($con, "select id_trabajador, nombres, paterno, materno from man_trabajadores order by paterno, materno, nombres");
						if (mysqli_num_rows($consulta) > 0){
							while ($row = mysqli_fetch_array($consulta)){
                    ?>
			                    <option value="<?php echo $row[id_trabajador];?>" <?php if($row[id_trabajador]==$administrador){ echo 'selected';}?>><?php echo strtoupper($row[nombres]).' '.strtoupper($row[paterno]).' '.strtoupper($row[materno]);?></option>
                    <?php }}?>
                  </select>
                </div>
              ...
              <div class="box-footer">
                ...
                <button type="button" class="btn btn-default" onClick="javascript:location.href='man_sucursales0.php'">Cancelar</button>
...
<script>
	$(document).ready(function () {
		...
		$("#txtNombre").keyup(function(){
			...
			nombre2 = $("#txtTelefono").val();
			if(nombre1.length>=10 && nombre2.length>=6){
