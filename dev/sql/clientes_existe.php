<?php
	...
	{
		if($id>0){
			$consulta = mysqli_query($con, "select nombre from man_clientes where id_tipodoc = '$tipo' AND nro_doc = '$valor' and id_cliente<>'$id'");
		}else{
			$consulta = mysqli_query($con, "select nombre from man_clientes where id_tipodoc = '$tipo' AND nro_doc = '$valor'");
		}
		if (mysqli_num_rows($consulta) > 0){
			echo "<span>El cliente ya existe.</span>";
