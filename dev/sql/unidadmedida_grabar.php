<?php
	...
	if($oculto=="41846788"){
		...
		if($grabar>0){
			$consulta = mysqli_query($con, "update man_unidadmedida set nombre='$txtNombre', user_modifica='$vgCodUser', fec_modifica=now() where id_umed='$grabar'");
		}else{
			$consulta = mysqli_query($con, "insert into man_unidadmedida(nombre, user_registro, fec_registro) values('$txtNombre', '$vgCodUser', now())");
		}
		if($consulta){
